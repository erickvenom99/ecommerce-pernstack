...
export default function AdminStores() {
    ...
    const fetchStores = async () => {
        try {
            ...
            const res = await axios.get('/api/admin/stores', {headers: {Authorization: `Bearer ${token}`}})
            setStores(res.data.ApproveStores)
    ...
    const toggleIsActive = async (storeId) => {
        try {
            const token = await getToken()
            const res = await axios.post('/api/admin/toggle-store', {storeId}, {headers: {Authorization: `Bearer ${token}`}})
            await fetchStores()
